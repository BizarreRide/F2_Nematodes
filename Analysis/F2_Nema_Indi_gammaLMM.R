...
nema <- subset(nema, select=c("MI", "MI25", "N"))
# 
...
p.indi.gammalmer <- matrix(NA,3,2+p)
colnames(p.indi.gammalmer) <- c("Env", "DF", colnames(nema)[1:p])

f.indi.gammalmer <- p.indi.gammalmer
...
for(i in 1:p) {
  ...
  model <- glmer(y ~ age_class*samcam +(1|field.ID), family=Gamma, indices2)
...
names(indi.gammalmer)[1:p] <- mod.names

indi.gammarsquared <- matrix(NA,2,p)
row.names(indi.gammarsquared) <- c("R2m", "R2c")
colnames(indi.gammarsquared) <- colnames(nema)

for(i in 1:p) {
  indi.gammarsquared[,i] <- MuMIn::r.squaredGLMM(indi.gammalmer[[i]])
}

# save(list=c("f.indi.gammalmer","p.indi.gammalmer","indi.gammarsquared"), file="Results/CHi2+p+r2_IndigammaLMM.rda")
# write.csv(f.indi.gammalmer, file="Results/Chi2_IndigammaLMM.csv")
# write.csv(p.indi.gammalmer, file="Results/p_IndigammaLMM.csv")
# write.csv(indi.gammarsquared, file="Results/r2_IndigammaLMM.csv")
...
indi.gammaLettersAS <- matrix(NA,8,p)
indi.gammaestimates <- matrix(NA,8,p)
indi.gammapairs <- matrix(NA,8,4*p)
indi.gammalsm <- list()
colnames(indi.gammaLettersAS) <-  colnames(nema)[1:p]
colnames(indi.gammaestimates) <-  colnames(nema)[1:p]
colnames(indi.gammapairs) <- rep(colnames(nema)[1:p], each=4)

for (i in 1:p) {
  # get the results on a back transformed scale:
  lsm <- lsmeans(indi.gammalmer[[i]],   ~ age_class*samcam , "cld")
  summary(lsm)
  #lsm <- lsmeans(indi.gammalmer[[i]],   ~ age_class*samcam, adjust="tukey",contr= "cld")
  x <- cld(lsm, type = "response")
  indi.gammaLettersAS[,i] <- x$".group"
  indi.gammaestimates[,i] <- x$"lsmean"
  indi.gammapairs[,((4*i)-3)] <- x$"age_class"
  indi.gammapairs[,((4*i)-2)] <- x$"samcam"
  indi.gammapairs[,((4*i)-1)] <- x$"lsmean"
  indi.gammapairs[,((4*i)-0)] <- x$".group"
  print(cld(lsm, type = "response"))
  name <- paste("lsm",i,names(indices2)[i], sep = ".")
  indi.gammalsm[[i]] <- assign(name, lsm)
...
#  save(list=c("f.indi.gammalmer","p.indi.gammalmer","indi.gammarsquared","indi.gammaLettersAS", "indi.gammaestimates", "indi.gammapairs"), file="Results/CHi2+p_indi.gamma_bnGLMM.rda")
#  write.csv(indi.gammaLettersAS, file="Results/letters_indi_gammaLMM.csv")
#  write.csv(indi.gammaestimates, file="Results/estim_indi_gammaLMM.csv")
#  write.csv(indi.gammapairs, file="Results/pairs_indi_gammaLMM.csv")

for(i in 1:p){
  lsmip(indi.gammalsm[[i]], age_class ~ samcam, type = "response")
  print(summary(pairs(indi.gammalsm[[i]]), type = "response"))
  print(summary(pairs(regrid(indi.gammalsm[[i]])), type = "response"))
}
...
indi.gammalsmAC <- list()

for (i in 1:p) {
  # get the results on a back transformed scale:
  lsm <- lsmeans(indi.gammalmer[[i]], pairwise ~ age_class|samcam)
  print(summary(lsm, type = "response"))
  name <- paste("lsm",i,names(nema)[i], sep = ".")
  indi.gammalsmAC[[i]] <- assign(name, lsm)
}

#************************************************************************


indi.gammalsmSC <- list()

for (i in 1:p) {
  # get the results on a back transformed scale:
  lsm <- lsmeans(indi.gammalmer[[i]], pairwise ~ samcam|age_class)
  print(summary(lsm, type = "response"))
  name <- paste("lsm",i,names(nema)[i], sep = ".")
  indi.gammalsmSC[[i]] <- assign(name, lsm)
}
...
for(k in 1:p){ 
  # print(list(summary(indi.gammalmer[[k]]),Anova(indi.gammalmer[[k]], type="III")))
  #corvif(indi.gammalmer[[k]])
  
  E1 <- resid(indi.gammalmer[[k]], type="pearson")
  E2 <- resid(indi.gammalmer[[k]], type="response")
  F1 <- fitted(indi.gammalmer[[k]], type="response")
  P1 <- predict(indi.gammalmer[[k]], type="response")
  ...
  title(names(indi.gammalmer)[k], outer=TRUE)
...
infl <- list()
for (k in 1:p) {
  infl[[k]] <- influence(indi.gammalmer[[k]], obs=TRUE)
}
...
for(i in 1:p) {
  indices2 <- indices[outlier[[i]],]
  indices2$y <- nema[outlier[[i]],i]
  model <- glmer(y ~ agsam + (1|field.ID),family=Gamma, indices2) # No Difference
...
for(i in 1:p) {
  ...
  testdata$fit <- X %*% fixef(indi.gammalmer[[i]])
  testdata$SE <- sqrt(  diag(X %*%vcov(indi.gammalmer[[i]]) %*% t(X))  )
