...
d <- indices2
d$Timef <- indices2$samcam
d$Time <- as.numeric(indices2$samcam)
d$Program <- indices2$age_class
d$Strength <- indices2$PPI
d$Subj <- indices2$field.ID
...
coef(d.timer)
prog=rep(1:3,table(d$Program)/7)
