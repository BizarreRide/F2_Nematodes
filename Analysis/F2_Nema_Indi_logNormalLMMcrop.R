...
df.backup <- cbind(FaPro[,-c(1:5)], MaturityIndices, biodiv)
...
df.response <- subset(df.response, select=c("BI","CI", "PPI", "PPI.1", "MI", "MI25", "N"))
...
for(i in 1:p) {
  ...
  df.Fpvalue[2:(1+q),2+((i*2)-1)] <- round(car::Anova(model, type="II")$"LR Chisq",2)
...
# save(list=c("df.FpvalueR2"), file="Results/ANOVATables/FpR2_Indi_logLMM_crop.rda")
# write.csv(df.FpvalueR2, file="Results/ANOVATables/FpR2_Indi_logLMM_crop.csv")
...
for(i in 1:p){
  indices2 <- indices[outlier[[i]],]
  indices2$y <- df.response[outlier[[i]],i]
  obj.afex <- afex::mixed(y ~ age_class , family = gaussian(link="log"), indices2, method="LRT") 
  df.FpvalueR2.1[2:(1+q),2+(i*2)-1] <- round(obj.afex[[1]]$"Chisq",2)
  df.FpvalueR2.1[2:(1+q),2+((i*2))] <- round(obj.afex[[1]]$"Pr(>Chisq)",3)
}

# write.csv(df.FpvalueR2.1, file="Results/ANOVATables/FpR2afex_Indi_logLMM_crop.csv")
...
# write.csv(df.posthoc, file="Results/ANOVATables/PostHocC_Indi_logLMM_crop.csv")
# write.csv(df.posthoc2, file="Results/ANOVATables/PostHocAC_Indi_logLMM_crop.csv")
# write.table(df.posthoc, file="Results/ANOVATables/FpR2_Indi_logLMM.csv", append=TRUE, sep=",", dec=".", qmethod="double", col.names=NA)
# write.table(df.posthoc2, file="Results/ANOVATables/FpR2_Indi_logLMM.csv", append=TRUE, sep=",", dec=".", qmethod="double", col.names=NA)

#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


### log-normal GLM - Model Validation ####
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
