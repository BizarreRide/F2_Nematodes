...
#write.csv(df.FpvalueR2, file="Results/ANOVATables/FpR2_Indi_LMMT2.csv")
write.csv(df.FpvalueR2, file="Results/ANOVATables/FpR2_Indi_LMMT3.csv")
...
for(i in 1:p){
  ...
  df.FpvalueR2.1[2:(1+q),2+(i*2)-1] <- round(obj.afex[[1]]$"F",2)
  df.FpvalueR2.1[2:(1+q),2+((i*2))] <- round(obj.afex[[1]]$"Pr(>F)",3)
}
df.FpvalueR2.1[1,] <- c("X", "X", rep(c("KR-F", "p-value"),p))

write.csv(df.FpvalueR2.1, file="Results/ANOVATables/FpR2afex_Indi_LMM.csv")
write.csv(df.FpvalueR2.1, file="Results/ANOVATables/FpR2afex_Indi_LMM2.csv")
...
save(list=c("ls.models","ls.lsm", "ls.lsmAC", "ls.lsmSC",  "df.FpvalueR2", "df.posthoc", "df.posthocAC", "df.posthocSC"), file="Results/ANOVATables/Indi_LMM.rda")
write.csv(df.posthoc, file="Results/ANOVATables/PostHoc_Indi_LMM.csv")
write.csv(df.posthocAC, file="Results/ANOVATables/PostHocAC_Indi_LMM.csv")
write.csv(df.posthocSC, file="Results/ANOVATables/PostHocSC_Indi_LMM.csv")
write.table(df.posthoc, file="Results/ANOVATables/FpR2_Indi_LMM.csv", append=TRUE, sep=",", dec=".", qmethod="double", col.names=NA)
write.table(df.posthocAC, file="Results/ANOVATables/FpR2_Indi_LMM.csv", append=TRUE, sep=",", dec=".", qmethod="double", col.names=NA)
write.table(df.posthocSC, file="Results/ANOVATables/FpR2_Indi_LMM.csv", append=TRUE, sep=",", dec=".", qmethod="double", col.names=NA)
